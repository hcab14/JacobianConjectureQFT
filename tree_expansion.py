...
# ---------------------------------------------------------------------------
# Truncated sparse polynomial arithmetic: {exponent tuple: Rational coeff}
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Composition F(phi) in the truncated ring
# ---------------------------------------------------------------------------
...
def compose_F(phi, N, nsrc):
    ...
    for fd in F_DICTS:
        ...
        for (ex, ey, ez), coeff in fd.items():
            term = pscale(pmul(pmul(pows[0][ex], pows[1][ey], N), pows[2][ez], N), coeff)
            acc = padd(acc, term)
        out.append(acc)
    return tuple(out)
...
def formal_inverse(J, N, nsrc, max_iter=None):
    ...
    for _ in range(max_iter):
        err = tuple(padd(Ji, pscale(Fi, -1)) for Ji, Fi in zip(J, compose_F(phi, N, nsrc)))
