...
- **$y$ and $z$ never escape.** Their minimal cubics over $\mathbb{C}[a,b,c]$
  ...
  $x$-direction).
- **Discriminant classified.** $\mathrm{disc}_X = -p\,(4q^3 + 27 p r^2)$.
...
- **No finite-module version, provably:** $x$ is NOT integral over
  $\mathrm{im}F^*$ — exact escape certificate
...
- **Explicit variational counterexamples in dimension 6** — to our
  ...
  `PROBLEM.md`): $\det\mathrm{Hess}W_6 \equiv -4$, degree 8,
  ...
  with $\det(I + \mathrm{Hess}f_H) \equiv 1$ (proved via a
...
- **Strongest 3D no-go for AM:** $\{K : K\,JF(u)$ symmetric
  ...
  ($\mathrm{tr}(C\,JF) = 3$ is an inconsistent linear system).
...
- **Coercivity synthesis:** coercive nondegenerate $W$ forces
  ...
  $\bar\varphi$); $\det\mathrm{Hess}W$ = const forces the leading
...
- **Exact:** eliminant irreducible over $\mathbb{Q}(a,b,c)$,
  $\mathrm{disc}_X=-4\,D_0^2\,p$ not a square $\Rightarrow$
  $\mathrm{Gal}=S_3$.
